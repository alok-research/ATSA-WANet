...
# Change the value inside the '# box' (line no. - 10, 14, 17, 21, 28, 32, 110-114, 119,120, 143-146, 151, 152) as per 
# Note: In case of older version of ns-allinone-xyz (earlier to 3.36.1), uncomment the program execution command in automation.sh line number 35 as './waf --run scrach/$iteration' and comment (by #) the program execution command in automation.sh line number 36 as './ns3 run scrach/$iteration'

path=$(pwd)
ns3_path=$path
 
#################################
ns3_version=3.36.1		 # change the installed 'ns-allinone-xyz' version (xyz) in which user wants to execute program
#################################
...
########################################
simulation_stop_time=250		## Simulation time
########################################
...
n_count=0 
########################
var_name=ART	 	# Change here variable name which will be display inside graphs. In present case it is ART(Active Route Timeout).
var_value_first=1   	# set the first value of parameter as display by 'parm_disc_display.sh'
var_value_last=3	# set the last value of parameter as display by 'parm_disc_display.sh'
var_value_diff=1	# set difference value between two value as display by 'parm_disc_display.sh'
for l in {1..3..1} 	# Change here value of variable as above- var_value_first, var_value_last, var_value_diff.
	do
	cd $ns3_path/ns-allinone-$ns3_version/ns-$ns3_version/scratch

################################################################################################	
	sed -i '/aodv.Set/d' $user_program_name_cp					        # delete 'aodv.set' line from user program
	sed -i 's/.*InternetStackHelper.*/aodv.Set("ActiveRouteTimeout",TimeValue (Seconds ('$l')));\n&/' $user_program_name_cp	     # add line 
######################################################################################'aodv.Set(....'as display by 'parm_disc_display.sh'
...
######################################################################
	x_min=10							# minimum value of x-axis as display by 'parm_disc_display.sh'
	x_max=60							# maximum value of x-axis as display by 'parm_disc_display.sh'
	x_delta=10							# delta difference
...
mv "$ns3_path"/$directory/*.png "$ns3_path"/$directory/$graph_directory
echo
echo "*** graphs have been generated till this statistical data and stored in directory- $graph_directory"		
